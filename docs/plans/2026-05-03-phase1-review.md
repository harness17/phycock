...
| F | CommonLibrary HtmlHelper 移植 | Codex | ✅ 完了（2026-05-03） |
...
## フェーズ F: CommonLibrary HtmlHelper 拡張メソッド移植（✅ 完了 Codex 2026-05-03）
...
- [x] `CommonLibrary/Extensions/Helper/` フォルダを新規作成
- [x] `CommonLibrary/Extensions/Helper/HtmlExtensions.cs`（新規）
...
- [x] `CommonLibrary/Extensions/Helper/HtmlExtensionsForCheckBox.cs`（新規）
...
- [x] `CommonLibrary/Extensions/Helper/HtmlExtensionsForRadioButton.cs`（新規）
  - `RadioButtonForEnum<TModel, TProperty>` — `orderDecFlag=true` で逆順対応
  - `RadioButtonForSelectList<TModel, TProperty>`
- [x] `Phycock/Views/_ViewImports.cshtml` に `@using Dev.CommonLibrary.Extensions.Helper` を追加
- [x] `dotnet build` で通過確認
...
- [x] フェーズ F（HtmlHelper 移植）完了後に `dotnet build` 再確認
